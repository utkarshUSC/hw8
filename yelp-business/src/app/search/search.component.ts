...
export class SearchComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.searchCtrl.valueChanges
      .pipe(
        ...
        switchMap(value => this.http.get('http://myapp:8000/get_autocomplete_words' + '?kw=' + value)
          .pipe(
            finalize(() => {
  ...
  async prepareReview(bId: string) {
    var resp = await axios.get("http://myapp:8000/get_business_reviews", { params: { 'id': bId } });
    if (resp.status != 200) {
  ...
  async prepareDataset(bId: string, bName: string) {
    var divTable = document.getElementById('searchTable') as HTMLTableElement;
    divTable.classList.add('d-none');
    var resp = await axios.get("http://myapp:8000/get_business_info", { params: { 'id': bId } });
    if (resp.status != 200) {
      alert('Sorry! Not much information available on this business');
      return;
    }
  ...
  async submit(form: NgForm) {
    ...
    var resp_business = await axios.get('http://myapp:8000/get_businesses/', { params: obj });
    var notFound = document.getElementById('noResult') as HTMLDivElement;
